...
$(document).ready(function () {
  ...
  $("#headerArea").mouseleave(function () {
    ...
    if (scroll < smh - 80) {
      $(this).css("background", "#fff").css("border-bottom", "none");
      $(".dropdownmenu li a").css("color", "#050D1B");
      $(".top_menu li a").css("color", "#050D1B");
    } else {
      $(this).css("background", "#fff").css("color", "#050D1B");
      $(".dropdownmenu li a").css("color", "#050D1B");
      // $('.top_menu li a').css('color', '#050D1B');
    }
    on_off = false;
  ...
  $(window).on("scroll", function () {
    ...
    if (scroll > smh - 80) {
      ...
      $("#headerArea")
        .css("background", "#fff")
        .css("box-shadow", "0 0 8px rgba(0, 0, 0, .3)");
      $(".dropdownmenu li a").css("color", "#050D1B");
      $(".top_menu li a").css("color", "#050D1B");
    } else {
  ...
  $("ul.dropdownmenu").hover(
    function () {
      $("ul.dropdownmenu li ul").fadeIn("normal", function () {
        $(this).stop();
      }); //모든 서브를 다 열기
...
$(document).ready(function () {
  $("#footerArea .familysite .arrow").toggle(
    function () {
      $("#footerArea .familysite .alist")
        .css("transform", "translateY(-100%)") // 초기 위치를 아래로 설정
        .stop(true, true)
        .slideDown(2000, "easeOutBounce"); // 올리는 효과
      $(this).children("span").html('<i class="fa-solid fa-chevron-down"></i>');
    },
    function () {
      $("#footerArea .familysite .alist")
        .stop(true, true)
        .slideUp(500, "easeOutQuad"); // 내리는 효과
